...
export const gmailTools: RealtimeTool[] = [
  ...
  },
  {
    type: "function",
    name: "search_emails",
    description: "Search Gmail using Gmail query syntax. Supported filters: is:unread, is:read, newer_than:1d (last 24 hours), newer_than:6h, after:YYYY/MM/DD, before:YYYY/MM/DD, from:email, subject:text, in:inbox. Examples: 'is:unread newer_than:1d', 'is:unread after:2026/08/01', 'from:alice@company.com'. To find mail from a company/sender, use from: with the company name or any word in it — e.g. from:Capital One or from:capitalone (Gmail matches the sender's display name and address). Do NOT guess full email domains like from:capitalone.com; the real domain often differs (e.g. notification.capitalone.co.uk) and will return nothing. If the user gives you the sender's email address, use from:<full-address>. Use this for ANY request about unread emails, recent emails (last N hours/days), or filtered email reviews. Read ALL emails returned before answering; do not summarize only the newest few unless they are the only matches. If the result includes a nextPageToken and there may be more matching emails, call again with that pageToken and keep reading until there are no more pages.",
    parameters: {
      type: "object",
      properties: {
        query: { type: "string" },
